...
<section>
  <body>
  	<form name="form1" method="post" action="php/event_reg_action.php">
      <?php
      include 'php/config.php';
      $id=$_SESSION["tech_name"][username];
      $query="SELECT event_no FROM stud_event WHERE username='$id'";
      $result=mysql_query($query);
      $query3="SELECT COUNT(*) FROM events";
      $result3=mysql_query($query3);
      ?>
      <div id="apDiv9">Events</div>
      <div id="apDiv10">
        <table width="100%">
        <tr>
       ...
       <?php
      			$i=0;
            $valu=mysql_fetch_array($result3);
            while ($info=mysql_fetch_array($result,MYSQL_ASSOC)) {
              $t[]=$info[event_no];
            }
            for($v=1;$v<=$valu['COUNT(*)'];$v++) {
              $in[]=$v;
            }
            if(isset($t)) {
              $in=array_diff($in,$t);
      ...
      ?>
      <tr>
      <td><?php echo $info2[$i][event_no] ?></td>
  	...
  	<label>
  		<button type="submit" id="btn2" name="submit" class="btn btn-success">Register</button>
  	</label>
</section>
